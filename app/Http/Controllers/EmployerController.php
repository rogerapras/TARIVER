...
class EmployerController extends Controller
{
    ...
    public function employerDashboard()
    {
        if (Auth::user()) {
            ...
            $commision_em = !empty($currency[0]['commision_em']) ? $currency[0]['commision_em'] : null;
    ...
    public function payoutSettings()
    {
        if (Auth::user()) {
            ...
            if (file_exists(resource_path('views/extend/back-end/employer/payouts/payout_settings.blade.php'))) {
                return view(
                    'extend.back-end.employer.payouts.payout_settings',
                    compact('payrols', 'payout_settings')
                );
            } else {
                return view(
                    'back-end.employer.payouts.payout_settings',
                    compact('payrols', 'payout_settings')
                );
            }
        } else {
